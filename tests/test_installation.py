...
class InstallationTests(unittest.TestCase):
    def test_built_wheel_console_command_enforces_total_input_budget(self):
        ...
        try:
            ...
            paths = [os.path.join(temporary, name) for name in ("reference.env", "target.env")]
            for path in paths:
                with open(path, "w") as stream:
                    stream.write("A=one\n")
            site = next(directory for directory, _, _ in os.walk(prefix) if directory.endswith("site-packages"))
            result = subprocess.run([os.path.join(prefix, "bin", "envdiff"), "--total-bytes", "10"] + paths, env=dict(os.environ, PYTHONPATH=site), stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            self.assertEqual(result.returncode, 2)
            self.assertEqual(result.stdout, "")
        finally:
            shutil.rmtree(temporary)
    def test_built_wheel_console_command_supports_bounded_output_file(self):
        root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        temporary = tempfile.mkdtemp()
        try:
            wheel_directory = os.path.join(temporary, "wheel")
            ...
            output = os.path.join(temporary, "report.txt")
            for path, contents in ((reference, "A=one\n"), (target, "A=two\n")):
                with open(path, "w") as stream:
                    stream.write(contents)
            site = next(directory for directory, _, _ in os.walk(prefix) if directory.endswith("site-packages"))
            result = subprocess.run([os.path.join(prefix, "bin", "envdiff"), "--max-bytes", "16", "--output", output, reference, target], env=dict(os.environ, PYTHONPATH=site), stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            self.assertEqual(result.returncode, 1)
            with open(output) as stream:
                self.assertEqual(stream.read(), "CHANGED A\n")
        finally:
            shutil.rmtree(temporary)
    def test_built_wheel_console_command_writes_json_report(self):
        root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        temporary = tempfile.mkdtemp()
        try:
            wheel_directory = os.path.join(temporary, "wheel")
            ...
            for path, contents in ((reference, "A=one\n"), (target, "A=two\n")):
                with open(path, "w") as stream:
                    stream.write(contents)
            site = next(directory for directory, _, _ in os.walk(prefix) if directory.endswith("site-packages"))
            result = subprocess.run([os.path.join(prefix, "bin", "envdiff"), "--json", reference, target], env=dict(os.environ, PYTHONPATH=site), stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            self.assertEqual(result.returncode, 1)
    ...
    def test_fresh_source_archive_runs_core_and_cli_tests(self):
        ...
        try:
            ...
            source = os.path.join(temporary, "envdiff-0.6.0")
    ...
    def test_package_metadata_identifies_project(self):
        ...
        result = subprocess.run([sys.executable, "setup.py", "--name", "--version"],
                                ...
                                universal_newlines=True)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stdout.splitlines(), ["envdiff", "0.6.0"])
